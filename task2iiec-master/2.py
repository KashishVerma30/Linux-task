...
while True:
  ...
  if("hello" in ch) or ("hi" in ch) and ("heya" in ch) or ("hey" in ch):
    ...
    pyttsx3.speak("i am your voice assistant boss, i am very intelligent , and i can control linux server for you")
  elif("date" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=date")
    pyttsx3.speak("showing date")
  elif("calendar" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=cal")
    pyttsx3.speak("showing calendar")
  elif("IP" in ch )or ("ip address" in ch) or "address" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=ifconfig")
    pyttsx3.speak("showing IP address")
  elif("active ports" in ch) or ("ports" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=netstat%20-tnlp")
    pyttsx3.speak("showing active ports")
  elif("amount" in ch) or ("disc" in ch) and ("space" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=df%20-h")
    pyttsx3.speak("showing amount of disc space")
  elif("status" in ch) or ("webserver" in ch) and ("apache" in ch) or ("httpd" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=systemctl%20status%20httpd")
    pyttsx3.speak("apache webserver is running")
  elif("state of selinux" in ch) or ("selinux" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=getenforce")
    pyttsx3.speak("SElinux is permissive")
  elif("yum" in ch) or ("module" in ch) and ("list" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=yum%20module%20list")
    pyttsx3.speak("Here is the long list of modules")
  elif("package" in ch) or ("firefox" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=yum%20whatprovides%20firefox")
    pyttsx3.speak("Here is the package name of firefox")
  elif("ping" in ch) or ("google" in ch) and ("server" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=ping%20-c%205%208.8.8.8")
    ...
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=rpm%20-q%20docker-ce")
    pyttsx3.speak("docker is not installed....")
  elif("configure docker" in ch) or ("install" in ch) and ("configure" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=yum%20install%20docker-ce%20--nobest%20-y")
    pyttsx3.speak("installing docker....")
  elif("start service" in ch) or ("start docker servie") and ("service") in ch:
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=systemctl%20start%20docker%20")
    ...
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=docker%20ps")
    pyttsx3.speak("There is no docker containers")
  elif("how many" in ch) and ("docker images" in ch) or ("images" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=docker%20images")
    pyttsx3.speak("There are several images")
  elif("launch" in ch) and ("docker container" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=docker%20run%20-dit%20wordpress")
    ...
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=rpm%20-ivh%20 hadoop-1.2.1-1.x86_64.rpm%20--force")
    pyttsx3.speak("hadoop is successfully installed...")
  elif("create" in ch) or ("directory" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=mkdir%20/namenode")
    pyttsx3.speak("directory created")
  elif("setup or start" in ch) or ("namenode" in ch) or ("masternode" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=hadoop-daemon.sh%20start%20namenode")
    pyttsx3.speak("name node started")
  elif("report" in ch) or ("dfs" in ch) or ("admin" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=hadoop%20dfsadmin%20-report")
    pyttsx3.speak("here is the report of hadoop cluster")
  elif("SDK" in ch) or ("python library" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=pip3%20install%20boto3")
    pyttsx3.speak("boto three is already satisfied")
  elif("aws" in ch) or ("CLI" in ch) or ("command line" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=pip3%20install%20awscli")
    ...
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=yum%20install%20hping3%20-y")
    pyttsx3.speak("H ping three is successfully installed")  
  elif("user" in ch) or ("mick" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=useradd%20mickey")
    pyttsx3.speak("mick user has been created")  
  elif("restrict" in ch) or ("addy" in ch):
    wb.open("http://192.168.43.212/cgi-bin/local.py?x=useradd%20-s%20/usr/bin/cal%20addy")
    pyttsx3.speak("Boss user has been created and restricted") 
